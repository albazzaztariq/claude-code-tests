...
def extract_table_with_nougat(pdf_path: str, table_number: int):
    ...
    Calls nougat CLI and parses the markdown output.
    Returns tuple of (row_count, table_data) if found, None otherwise.
    """
    try:
        print(f"    Extracting with Nougat OCR (CLI)...")
        import subprocess
        import tempfile
        import shutil
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            # Copy PDF to temp location with simple name (avoid path issues with spaces)
            ...
            print(f"    Files in tmpdir: {list(Path(tmpdir).iterdir())}")
            if not mmd_files:
                print("    No .mmd output file generated by Nougat")
                return None

            # Read the markdown output
            markdown_text = mmd_files[0].read_text(encoding='utf-8')
            predictions = [markdown_text]
