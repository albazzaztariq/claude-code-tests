    ...
    try:
        ...
        def write_sheet(ws, papers, sheet_name):
            ...
            for row, paper in enumerate(papers, 2):
                ...
                # Breakpoint: "Download Failed" if failed, "Unknown File Format" if unknown, blank otherwise
                if status == "failed":
                    breakpoint_value = "Download Failed"
                elif status == "unknown":
                    breakpoint_value = "Unknown File Format"
                else:
                    breakpoint_value = ""  # File extension shows format (PDF, HTML, XML, JSON)
                ...
                if status in ["downloaded", "html", "xml", "json"]:
                    ...
                    fill = PatternFill(start_color="FFA500", end_color="FFA500", fill_type="solid")  # Orange (unknown format)
                    for c in range(1, 12):
                        ws.cell(row=row, column=c).fill = fill
                elif status == "failed":
        ...
        if success:
            if file_format:
                if file_format == "UNKNOWN":
                    # Unknown format - keep as .pdf but mark status
                    paper["status"] = "unknown"
                else:
                    # Rename file to correct extension (HTML, XML, JSON instead of PDF)
                    extension_map = {"HTML": ".html", "XML": ".xml", "JSON": ".json"}
                    correct_ext = extension_map.get(file_format, ".pdf")
                    new_filepath = filepath.with_suffix(correct_ext)
...
def main():
    ...
    excel_path = query_folder / "Corpus Metadata.xlsx"
    ...
    print("COMPLETE")
    print(f"{'='*60}")
    print(f"  Corpus Metadata: {excel_path}")
