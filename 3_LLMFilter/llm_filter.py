...
def filter_papers(pdf_folder: Path, excel_path: Path) -> Path:
    ...
    for row_idx in range(2, ws.max_row + 1):
        ...
        # Check existing Breakpoint value from Pipeline 1
        existing_breakpoint = ws.cell(row=row_idx, column=breakpoint_col).value

        # Skip if PDF was never successfully downloaded or has unknown format (from Pipeline 1)
        download_failures = ["Download Failed", "Unknown File Format"]
        if existing_breakpoint in download_failures:
            print(f"  [SKIP] PDF not processable (Breakpoint: {existing_breakpoint})")
            print(f"        Keeping original breakpoint value from Pipeline 1")
            # Keep row colored from Pipeline 1 (red for failed, orange for unknown)
            continue
...
def main():
    ...
    if args.pdf_folder is None:
        ...
        args.excel_file = query_folder / "Corpus Metadata.xlsx"
