...
def extract_sample_count_from_table(pdf_path: str, full_text: str) -> int:
    ...
    print("    - Group 1 + Group 2 + Group 3 (BEST)")
    print("    - Group 2 + Group 3 (COMMON)")
    print("    - Group 1 + Group 2 (COMMON)")
    print("    - Group 1 + Group 3 (RARE)")

    explicit_count = None
    sentences = re.split(r"[.!?]\n", search_text)
    print(f"    Split text into {len(sentences)} sentences")

    # Number word to digit mapping - EVERY NUMBER 1-50
    word_to_num = {
        "one": 1,
        "two": 2,
        "three": 3,
    ...
    for i, sentence in enumerate(sentences):
        ...
        # CHECK GROUP 1: "total of" OR number immediately before group 2
        has_total_of = "total of" in sentence_lower or "total" in sentence_lower
        
        # Check if number immediately precedes group 2 words
        word_pattern_temp = "|".join(word_to_num.keys())
        has_number_before_group2_check = False
        # Allow optional words between number and group2: "nine different types of fabrics"
        if (re.search(r"(?:^|[^\d.])(\d+)\s+(?:\w+\s+){0,6}(?:fabrics?|materials?|samples?|variants?|garments?|textiles?|specimens?)\b", sentence_lower) or
            re.search(rf"\b({word_pattern_temp})\s+(?:\w+\s+){{0,6}}(?:fabrics?|materials?|samples?|variants?|garments?|textiles?|specimens?)\b", sentence_lower)):
        ...
        # ADDITIONAL CONSTRAINT: Must have either "table" OR a number IMMEDIATELY before group 2 words
        has_table = "table" in sentence_lower
        
        # Check if there's a number IMMEDIATELY before group 2 words
        has_number_before_group2 = False
        
        # Build word pattern first
        word_pattern = "|".join(word_to_num.keys())
        
        # Check for digits IMMEDIATELY before group 2: "10 fabrics", "2 samples"
        # Use negative lookbehind to ensure not preceded by digit or decimal point
        if re.search(r"(?:^|[^\d.])(\d+)\s+(?:fabrics?|materials?|samples?|variants?|garments?|textiles?|specimens?)\b", sentence_lower):
            has_number_before_group2 = True
        
        # Check for word numbers IMMEDIATELY before group 2: "ten fabrics", "two samples", "nine fabrics"
        if re.search(rf"\b({word_pattern})\s+(?:fabrics?|materials?|samples?|variants?|garments?|textiles?|specimens?)\b", sentence_lower):
            has_number_before_group2 = True
        
        if not has_table and not has_number_before_group2:
            print(f"\n      Sentence {i}: VALID COMBINATION but REJECTED - No 'table' and no number IMMEDIATELY before group 2")
            continue
        ...
        digit_patterns = [
            r"total\s+of\s+(\d+)\s+(?:different\s+)?(?:fabrics?|materials?|samples?|variants?|garments?|textiles?|specimens?)",
            r"(?:^|[^\d.])(\d+)\s+(?:different\s+)?(?:fabrics?|materials?|samples?|variants?|garments?|textiles?|specimens?)",  # Not preceded by digit or decimal
            r"(?:tested|produced|used|analyzed|evaluated|studied|prepared|examined)\s+(\d+)\s+(?:fabrics?|materials?|samples?|variants?|garments?|textiles?|specimens?)",
        ]
        ...
        # ===== PRIORITY 2: WORD NUMBERS (one, two, three, etc.) =====
        word_pattern = "|".join(word_to_num.keys())
        word_patterns = [
            rf"\b({word_pattern})\b\s+(?:different\s+)?(?:fabrics?|materials?|samples?|variants?|garments?|textiles?|specimens?)",
            rf"\b({word_pattern})\b\s+(?:fabrics?|materials?|samples?|variants?|garments?|textiles?|specimens?)\s+of",
        ]
        ...
        # Roman numerals must be surrounded by spaces/punctuation, not part of alphanumeric codes
