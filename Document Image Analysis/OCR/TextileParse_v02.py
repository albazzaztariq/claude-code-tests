"""
SciMasterScrape2_test.py

IMPROVED VERSION: Comprehensive textile engineering data extraction from research PDFs.
Extracts detailed yarn, fabric, material, and performance metrics with minimal training.

KEY IMPROVEMENTS over v1:
- Multi-stage extraction (metadata → sample count → individual samples → specs)
- Better prompts with specific instructions for each extraction stage
- Robust JSON parsing with fallback handling
- Uses more context for metadata extraction
- Section-aware extraction (looks in Methods, Results sections)

Architecture:
- Stage 1: PDF preprocessing with Nougat (Neural Optical Understanding) or PyMuPDF
    ...
    def extract_with_nougat(self, pdf_path: str) -> str:
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                # Run Nougat inference on this page
                predictions = model.inference(image=img, batch_size=1)

                if predictions and len(predictions) > 0:
                    all_text.append(predictions[0])

            doc.close()

            if all_text:
                markdown_text = "\n\n".join(all_text)
                print(f"✓ Nougat extracted {len(markdown_text)} characters from {len(all_text)} pages")
